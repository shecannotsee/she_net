...
std::string she_net::basic_io_operations::UDP::recvfrom(int fd) {
  ...
  ssize_t bytes_read = ::recvfrom(fd, buffer, buffer_size - 1, NULL, (struct sockaddr *)&senderAddr, &senderAddrLen);
  /*z*/ if (bytes_read == -1) {
    if (errno == EINTR) {
      return {};
...
void she_net::basic_io_operations::UDP::sendto(int fd, std::string ip, std::string port, std::string binary_package,
  ...
  for (bool loop = true; loop == true;) {
    /*z*/ if (type == TRANSPORT_ADDRESS_TYPE::UDP_IPV4) {
      ...
      bytes_sent = ::sendto(fd, binary_package.c_str(), binary_package.size(), NULL, (struct sockaddr *)&server_address,

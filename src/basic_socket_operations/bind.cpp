...
namespace she_net {
void bind_t::handle_error(const int status) {
  if (status == 0) {
    // success
    return;
  } else if (status == -1) {
    const auto error_info = static_cast<bind_param::error_info>(errno);
    if (const auto it = bind_param::error_info_map.find(error_info); it != bind_param::error_info_map.end()) {
